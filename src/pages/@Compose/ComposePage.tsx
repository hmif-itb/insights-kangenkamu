...
const joyrideSteps = [
  {
    ...
    content: (
      <>
        Ketik nama lengkap atau panggilan dan pilih penerima pesan. Kamu bisa
        memilih lebih dari satu orang.
      </>
    ),
    disableBeacon: true,
  },
  {
    target: ".sender",
    content: <>Kamu bisa memilih untuk mengirim pesan secara anonim</>,
    disableBeacon: true,
  },
  {
    target: ".text",
    content: <>Tulis pesan kamu maksimal 200 karakter</>,
    disableBeacon: true,
  },
  {
    target: ".send",
    content: <>Kirim pesan kamu dengan mengklik tombol ini!</>,
    disableBeacon: true,
  },
];
...
const ComposePage: React.FC = () => {
  ...
  const filterOption = (option: Option, inputValue: string) => {
    const matchName = option.data.name
      .toLowerCase()
      .includes(inputValue.toLowerCase());
    const matchNickname = option.data.nickname
      ? option.data.nickname.toLowerCase().includes(inputValue.toLowerCase())
      : false;
  ...
  const handleJoyrideCallback = (data: CallBackProps) => {
    ...
    if (status === STATUS.SKIPPED || status === STATUS.FINISHED) {
      window.localStorage.setItem("kangenkamu_intro", "true");
    }
  };

  useEffect(() => {
    if (!window.localStorage.getItem("kangenkamu_intro")) {
      setRunTutorial(true);
    }
  }, []);
  ...
  return (
    <div className="ComposePage">
      ...
      <div className="content">
        ...
        >
          <Box color="text.secondary">From</Box>
          <Box flex={1} px={1}>
        ...
        >
          <Box p={2}>
            ...
            <Box mt={1} color="text.secondary">
              <small>
                Apabila kamu memilih untuk menjadi anonim, tidak ada yang bisa
                mengetahui siapa pengirim pesan, termasuk admin.
              </small>
            </Box>
          </Box>
          <Box mb={3}>
        ...
        <Divider />
        <Box p={2} flex={1} display="flex" className="text">
          <InputBase
